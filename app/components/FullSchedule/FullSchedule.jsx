...
export default function FullSchedule({ schedule }) {
  ...
  schedule = schedule.filter((sessions) => sessions.sessions.length > 0);
  return (
    <div className={styles.ScheduleContainer}>
      ...
      {schedule
        .filter((day) => day.date === chosenDay || chosenDay === "all")
        .map((day, key) => (
              ...
              {day.sessions.map((session, key) => (
                <div key={key} className={styles.sessionContainer}>
                  ...
                  <p>
                    ...
                    {session.fromTime + " - " + session.endTime}
                  </p>
                  <p>
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                    ...
                    Given by:{" "}
                    {session.sessionPapers
                      .map(
                        (sessionPapers) =>
                          sessionPapers.paper.presenter.first_name +
                          " " +
                          sessionPapers.paper.presenter.last_name
                      )
                      .join(", ")}
                  </p>
                  <p>
                    ...
                    Assigned papers:{" "}
                    {session.sessionPapers
                      .map((paper) => paper.paper.paperTitle)
                      .join(", ")}
                  </p>
                </div>
              ))}
